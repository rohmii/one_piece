...
public class SnailMenuScreen extends AbstractContainerScreen<SnailMenuMenu> {
	...
	public void render(PoseStack ms, int mouseX, int mouseY, float partialTicks) {
		...
		if (RenderSnailProcedure.execute(world, x, y, z) instanceof LivingEntity livingEntity) {
			InventoryScreen.renderEntityInInventoryRaw(this.leftPos + 197, this.topPos + 91, 100, 13.5f + (float) Math.atan((this.leftPos + 197 - mouseX) / 40.0), (float) Math.atan((this.topPos + 41 - mouseY) / 40.0), livingEntity);
		}
	}
